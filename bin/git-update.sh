...
git submodule foreach -q --recursive 'echo Updating $name && git remote prune origin && git pull --no-squash && git checkout $(git config -f $toplevel/.gitmodules submodule.$name.branch || echo master)'
