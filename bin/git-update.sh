...
git submodule foreach -q --recursive 'echo Updating $name; branch=$(git config -f $toplevel/.gitmodules submodule.$name.branch || echo develop); current=$(git rev-parse --abbrev-ref HEAD); if [ "$current" = "$branch" ]; then git pull --no-squash; else git fetch origin $branch:$branch; fi'
